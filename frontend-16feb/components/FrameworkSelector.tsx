    ...
    <div className="flex gap-2 flex-wrap">
      {FRAMEWORKS.map((fw) => (
        <button
          ...
          className={`px-4 py-2 rounded-lg border transition-all font-medium text-sm ${selected === fw.id
              ? 'bg-blue-600 text-white border-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.4)]'
              : 'bg-[var(--card-bg)] text-[var(--text-secondary)] border-[var(--card-border)] hover:border-[var(--accent-blue)] hover:text-[var(--text-primary)]'
            }`}
        >
          {fw.name}
        </button>
